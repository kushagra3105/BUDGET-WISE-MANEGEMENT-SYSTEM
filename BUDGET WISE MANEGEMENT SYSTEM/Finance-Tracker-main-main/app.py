...
# --- Budget Analyzer Prompt ---
BUDGET_ANALYZER_PROMPT = (
    "You are Budget Analyzer AI Assistant. "
    "Your job is to help users analyze their spending, answer questions about their expenses, "
    "and give practical, actionable suggestions to save money based on their transaction data. "
    "Always be friendly, helpful, and focus on financial well-being. "
    "If the user asks for ways to save money, use their top spending categories to give specific advice. "
...
def get_top_spending_categories(user_id, limit=3):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT category, SUM(amount) FROM transactions WHERE user_id = ? GROUP BY category ORDER BY SUM(amount) DESC LIMIT ?",
        (user_id, limit)
    )
...
def chat():
    ...
    # If user is logged in, fetch their top spending categories for personalized advice
    user_id = session.get('user_id')
    category_summary = ""
    if user_id:
        top_categories = get_top_spending_categories(user_id)
        if top_categories:
            category_summary = "Your top spending categories are: " + ", ".join(
                [f"{cat}: ₹{amt:.2f}" for cat, amt in top_categories]
            ) + ". "
        else:
            category_summary = "You have not added any transactions yet. "

    # Build the prompt for Gemini
    prompt = (
        f"{BUDGET_ANALYZER_PROMPT}\n"
        f"{category_summary}\n"
        f"User: {user_message}"
    )
...
def login():
    if request.method == 'POST':
        # --- CAPTCHA is skipped for localhost and 127.0.0.1 ---
        if request.host.startswith('localhost') or request.host.startswith('127.0.0.1'):
            pass  # Skip CAPTCHA validation
        else:
